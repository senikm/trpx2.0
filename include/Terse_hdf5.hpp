...
public:
    ...
    Terse(C&& data, Terse_mode const mode = Terse_mode::Signed) noexcept {
    ...
    Terse(Iterator const data, size_t const size, unsigned int const block=12, Terse_mode mode = Terse_mode::Signed) noexcept :
    ...
    /**
     * @brief Converts the Terse object to a 2D vector of type T.
     *
     * This function extracts all frames stored in the Terse object and decompresses them into
     * a 2D vector of type T. Each inner vector corresponds to a single frame, and the outer
     * vector contains all the frames.
     *
     * @tparam T The type of the elements in the resulting 2D vector. This type must be arithmetic.
     * @return A 2D vector of type T where each inner vector represents a frame.
     *
     * @note If the Terse object is multi-threaded (`CONCURRENT` is not void), decompression is performed
     * concurrently. Otherwise, decompression proceeds sequentially.
     *
     * @example
     * Here is an example of using the `operator std::vector<std::vector<T>>` to decompress a Terse object:
     * \code{.cpp}
     * Terse<Concurrent> terse_obj;  // Assume this Terse object is pre-populated with compressed data.
     * std::vector<std::vector<int>> frames = terse_obj;
     *
     * for (size_t i = 0; i < frames.size(); ++i) {
     *     std::cout << "Frame " << i << " decompressed data:\n";
     *     for (const auto& value : frames[i]) {
     *         std::cout << value << " ";
     *     }
     *     std::cout << "\n";
     * }
    ...
    operator std::vector<std::vector<T>> () {
        if constexpr (std::is_same_v<CONCURRENT, void>) {
            std::vector<std::vector<T>> result(number_of_frames(), std::vector<T>(size()));
            for (std::size_t i = 0; i != result.size(); ++i)
                prolix(result[i], i);
            return result;
        }
        else {
            std::vector<std::future<std::vector<T>>> futures(number_of_frames());
            for (std::size_t i = 0; i != futures.size(); ++i)
                futures[i] = d_concurrent->background([this, i] { return prolix(std::vector<T>(size()), i); });
            std::vector<std::vector<T>> result(number_of_frames());
            for (std::size_t i = 0; i != result.size(); ++i)
     ...
     */
    template <typename Iterator>
    void insert(std::size_t const pos, Iterator const data, size_t const size, Terse_mode mode = Terse_mode::Signed) noexcept {
     ...
     */
    template <Container C>
    void insert(std::size_t const pos, C&& data, Terse_mode mode = Terse_mode::Signed) noexcept {
        if constexpr (requires(C& c) { c.dim(); }) {
        ...
        if constexpr (std::is_lvalue_reference_v<C&&>)
            d_terse_frames.insert(at, f_compress(mode, data.begin()));
        else if constexpr (std::is_same_v<CONCURRENT, Concurrent>)
            d_terse_frames.insert(at, d_concurrent->background([this, d = std::move(data), mode] { return f_compress(mode, d.begin()); }));
        else {
            d_terse_frames.insert(at, f_compress(mode, data.begin()));
     ...
     */
     template <typename Iterator>
    void push_back(Iterator const it, size_t const size, Terse_mode mode = Terse_mode::Signed) noexcept {
     ...
     */
    template <Container C>
    void push_back(C&& data, Terse_mode mode = Terse_mode::Signed) noexcept {
    ...
    */
    ...
    /**
     * @brief Unpacks the Terse data of the requested framde and stores it in the provided container.
     *
    ...
    C&& prolix(C&& container) noexcept {
        ...
        if constexpr (std::is_same_v<CONCURRENT, void>)
            for (std::size_t i = 0; i != number_of_frames(); ++i)
                prolix(container.data() + i * size(), i);
        else {
    ...
    C&& prolix(C&& container) noexcept {
        if (number_of_frames() == 1)
            prolix(std::forward<C>(container[0]), 0);
...
private:
    ...
    std::conditional_t<std::is_same_v<CONCURRENT, void>, std::monostate, std::shared_ptr<Concurrent>> d_concurrent = [] {
        if constexpr (std::is_same_v<CONCURRENT, void>) return std::monostate{};
        else return std::make_shared<Concurrent>(1);
    }();
    ...
    auto f_compress(Terse_mode mode, Iterator const data_begin) {
        d_prolix_bits = std::max(d_prolix_bits, 8 * static_cast<unsigned>(sizeof(*data_begin)));
