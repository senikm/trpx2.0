...
public:
    ...
    XML_element(Stream&& istream, std::string const& tag) :
        ...
        if (istream.good() && (get_tag.size() < std::string::npos) && (get_tag.size() > 2)) {
            if (get_tag[get_tag.size() - 2] == '/')
                d_attributes_str = get_tag.substr(d_tag.size(), get_tag.size() - 2);
            else {
                d_attributes_str = get_tag.substr(d_tag.size(), get_tag.size() - 1 - d_tag.size());
                const std::string end_tag = "</" + d_tag + ">";
                do {
                    this->push_back('\0');
                    istream.read(&(this->back()), 1);
                } while (istream.good() &&
                         (this->size() != std::string::npos) &&
                         (this->back() != '>') &&
                         !(this->size() >= end_tag.size() &&
                           this->compare(this->size() - end_tag.size(), end_tag.size(), end_tag) == 0));
    ...
    std::string attribute(std::string const& name) const {
        auto it = d_attributes_map.find(name);
        if (it != d_attributes_map.end())
            return it->second;
        else
            return std::string("");
    }

    // New method to get all attributes
    const std::map<std::string, std::string>& attributes() const {
        return d_attributes_map;
    }
...
private:
    ...
    std::string d_attributes_str; 
    std::map<std::string, std::string> d_attributes_map;

    void parse_attributes() {
        if (d_attributes_str.empty())
            return;

        std::istringstream iss(d_attributes_str);
        std::string token;
        while (iss >> token) {
